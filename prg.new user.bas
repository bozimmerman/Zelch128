!--------------------------------------------------
!- Monday, May 15, 2017 3:03:15 AM
!- Import of : 
!- c:\src\zelch128\prg.new user.prg
!- Unknown Machine
!--------------------------------------------------
30000 SYS15625:REM ============= NEW USER MODULE V1.0 ============= 09/22/90 =
30010 IFO(1)=1THENO(1)=0:GOTO32750:ELSEO$="New User Login":GOSUB250:F$="sys.new msg":U=D(1,0):D=D(1,1):GOSUB25:POKE2825,10:POKE2824,1:X=M(14)+1
30100 U$(8)=RIGHT$(STR$(M(1)),1)+"0"+RIGHT$(STR$(M(12)),1)+"000008O":U(4)=0:U(5)=0
30200 DO:X=X-1:LOOPUNTILUI$(X)=""ORUI$(X)="^"ORX=1:IFX=1THENO$=T$(43):GOSUB4:GOTO600
30300 GOSUB30400:GOSUB30700:GOSUB30900:GOSUB31100:GOSUB31300:GOSUB34200:GOTO31500
...
31500 U$(5)="A":U$(6)="9001010530":U$(7)="00000000000000000000000000":U$(9)=" 1":U(1)=M(10):U(2)=M(11):U(3)=BA:U$(10)="11111111111111111111111111"
31600 O$="{clear}{ct k}{black}1) Handle      : {white}"+U$(1)+"{ct k}{black}2) Real name   : {white}"+U$(3)+"{ct k}{black}3) Phone number:{white} "+U$(4):GOSUB4
31700 O$="{black}4) Password    : {white}"+U$(2)+"{ct k}{black}5) Columns     :{white}"+STR$(40+(40*VAL(MID$(U$(8),2,1)))):GOSUB4
31720 I=VAL(MID$(U$(8),5,1)):O$="{black}6) Auto-Pause  : {white}o"+MID$("ffn ",I*2+1,2):GOSUB4
31730 X=INSTR("ACDEIMTPO",MID$(U$(8),10,1))-1:IFX<0THENX=8
31740 O$="{black}7) Computer    : {white}"+MID$("Amiga     C=64      C=128     Apple     IBM/Clone Macintosh Atari     CP/M      Other     ",(X*10)+1,10):GOSUB4
31790 O$=T$(95):GOSUB4:POKE2959,I
31800 C$="1234567"+R$:GOSUB28:O$=A$:GOSUB4:IFI=8THEN31900:ELSEONIGOSUB30400,30700,30900,31100,31300,33900,34200:GOTO31600
31900 O$=T$(51):GOSUB4:I=0:DO:I=I+1:LOOPUNTILUI$(I)="^"ORUI$(I)="":W0=I:FORI=0TO50:TT$(I)="":NEXTI
32000 ID=W0:O$=T$(52):GOSUB4:C$=R$:GOSUB28:ID=0:O$=R$:GOSUB4
...
32500 CLOSE2:CLOSE1:O$=T$(53):GOSUB4:C$="YN":GOSUB28:O$=A$:GOSUB4:IFI=2THEN32100:ELSEO$=T$(54):GOSUB4:OPEN1,D(1,0),15
...
32700 ID=W0:UI$(ID)=U$(1):GOSUB400:PRINT#2,STR$(ID)+R$+O$(3)+R$+U$(3)+R$+U$(4)+R$+U$(2):IFW2THENFORI=1TOW2:PRINT#2,TT$(I):NEXTI:ELSEPRINT#2,R$R$
32710 CLOSE2:CLOSE1:O(6)=O(6)+1:GOTO2000
32750 GOSUB990:I=VAL(M3$):IFI=0THEN32800
32760 ONIGOSUB30400,31100,31300,33100,33900,34000,34200,34100:UI$(ID)=U$(1):GOSUB100:GOTO940
...
32820 O$="{black}5) Auto-Pause: {white}o"+MID$("ffn ",VAL(MID$(U$(8),5,1))*2+1,2)+"{ct k}{black}6) Line #s   : {white}o"+MID$("ffn ",VAL(MID$(U$(8),7,1))*2+1,2):GOSUB4
32830 X=INSTR("ACDEIMTPO",MID$(U$(8),10,1))-1:IFX<0THENX=8
32840 O$="{black}7) Computer  : {white}"+MID$("Amiga     C=64      C=128     Apple     IBM/Clone Macintosh Atari     CP/M      Other     ",(X*10)+1,10):GOSUB4
32845 O$=" N/A":IFBA=2400THENO$=STR$(PEEK(2582)-160)
32850 O$="{black}8) 2400Timing:{white}"+O$:GOSUB4
32900 O$=T$(95):GOSUB4:C$="12345678"+R$:GOSUB28:O$=A$:GOSUB4
32910 ONIGOSUB30400,31100,31300,33100,33900,34000,34200,34100:UI$(ID)=U$(1)
33000 IFI=9THENGOSUB100:GOTO940:ELSE32800
...
34220 C$="ACDEIMTPO":GOSUB28:O$=A$:GOSUB4:MID$(U$(8),10,1)=A$:RETURN
