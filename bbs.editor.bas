...
140 DATA "920000000Check for 100% flag match",2,"200000000Page sysop",1,"234000000Dos Command",3,"734200000Directory",4
150 DATA "200000000Random Check",1,"C00000000Edit stats",1,"000000000User list",0,"000000000Relogon",0
...
720 POKE2823,1:POKE2983,255:POKE2825,255:POKE2824,1:POKE2827,0:SYS8201:RETURN
...
1580 IFZ=0THEN1350:ELSEI$=F1$:GOSUB1420:IFI$=""THEN960:ELSEF1$=I$:OPEN15,U,15,"s"+MID$(STR$(D),2)+":"+F1$:GOSUB940:OPEN8,U,8,MID$(STR$(D),2)+":"+F1$+",s,w":PRINT"{clear}";
...
2850 GOTO2560
2860 I$=F2$:GOSUB1420:IFI$=""THEN2560:ELSEF2$=I$:OPEN15,U1,15:OPEN2,U1,2,MID$(STR$(D1),2)+":"+F2$+",s,r":GOSUB940:IFA>0THENCLOSE2:CLOSE15:GOTO2560
...
2930 IFZ1=0THEN2540:ELSEI$=F2$:GOSUB1420:IFI$=""THEN2560:ELSEF2$=I$:D1$=MID$(STR$(D1),2):OPEN15,U1,15,"s"+D1$+":"+F2$:GOSUB940:OPEN8,U1,8,D1$+":"+F2$+",s,w"
...
3090 PRINT:I$="0=text not displayed when loaded,{ct k}1=text automatically displayed,":GOSUB2170:MD$(1)=LEFT$(MD$(1)+"0000000",1)+MID$(MD$(1),2)
3100 PRINT"{down}Old menu code: ";LEFT$(MD$(1),1):PRINT"     New code: ";:GOSUB720:IFI$=""THEN2560
3105 MID$(MD$(1),1,1)=MID$(STR$(VAL(I$)),2)
3110 I$="{ct k*2}Enter a command to be auto executed{ct k}when the menu is loaded. RETURN=None":GOSUB2170
3120 PRINT"{down}Current command: ";MID$(MD$(1),2);:I=ASC(MID$(MD$(1),2,1)):IFI=0THENPRINT"NONE":ELSEPRINT
3125 PRINT" Command is now: ";:GOSUB720:MD$(1)=LEFT$(MD$(1),1)+I$:IFI$=""THEN2560
3130 I$="{f6}The command that will be automatically{f7}executed is: "+MID$(MD$(1),2):GOSUB2170:GOTO2560
...
3190 IFZ1=0THENGOSUB2540:ELSEFORX=1TOZ1:PRINT"{down}{cyan}Command key: {white}";:I$=ME$(X,0):MAX=10:GOSUB4480
...
3310 PRINT"{cyan}Drive/LU     : "CL$;:GOSUB720:IFI$=""ORI$="*"ORI$="#"ORINSTR(I$,"/")>0THENRETURN
...
3370 PRINT"{cyan}Topic/Subboard (A - Z,#1 - #26): "CL$;:POKE2831,1:GOSUB720:IFI$=""ORI$="*"THENRETURN:ELSEIFLEFT$(I$,1)="#"ANDVAL(MID$(I$,2))THENRETURN
...
3445 I=1:DO:A=ASC(MID$(I$,I,1)):IFA>64ANDA<91THENMID$(I$,I,1)=CHR$(AOR128)
3446 I=I+1:LOOPUNTIL(I>LEN(I$)):RETURN
3447 PRINT"{down}No multi-key commands on a hotkey"+CHR$(13)+"BBS.  Check your system variables.":GOTO2560
3450 C0=0:PRINT:PRINT"Is this an op-code subroutine (y/n)? ";:GETKEYA$:IFINSTR("Yy",A$)>0THENC0=1:PRINT"Yes":ELSEPRINT"No."
3460 PRINT"{down}Command key: ";:GOSUB720:IFI$=""ORLEN(I$)>5THEN2560:ELSEIFVAL(M$(19))=0ANDLEN(I$)>1THEN3447:ELSEIFC0=0THENGOSUB3445
3470 Y=0:DO:Y=Y+1:LOOPUNTILME$(Y,0)=I$ORY>Z1:IFY>Z1THEN2560
3480 PRINT"{cyan}New command key or [RETURN] to keep: ";:GOSUB720:IFI$=""THEN3510:ELSEIFVAL(M$(19))=0ANDLEN(I$)>1THEN3447
3490 IFC0=0THENGOSUB3445
3500 I=0:DO:I=I+1:LOOPUNTILME$(I,0)=I$ORI=Z1:IFME$(I,0)=I$THENPRINT"{down}Command already used.":GOTO2560:ELSEME$(Y,0)=I$
3510 IFC0=0ANDVAL(M$(19))=0THENPRINT"{cyan}Description:":I$=ME$(Y,2):GOSUB1990:ME$(Y,2)=I$
...
3540 PRINT"{down}Command key: ";:GOSUB720:IFI$=""ORLEN(I$)>5THEN2560:ELSEIFVAL(M$(19))=0ANDLEN(I$)>1THEN3447:ELSEIFC0=0THENGOSUB3445
3550 X=0:DO:X=X+1:LOOPUNTILX>Z1ORI$=ME$(X,0):IFX<=Z1ANDI$=ME$(X,0)THENPRINT"{down}Command key already used.":GOTO2560
3560 X=0:DO:X=X+1:LOOPUNTILME$(X,0)=""ORX>Z1
3570 Y=X:ME$(Y,0)=I$:IFC0THENME$(Y,2)="(subroutine)":ELSEIFVAL(M$(19))=1THENME$(Y,2)="(input command)":ELSEPRINT"Description: ";:GOSUB720:ME$(Y,2)=I$
...
3670 WINDOW0,1,PEEK(238),24,1:PRINT"{clear}{down}{cyan}Command key: ";:I$=ME$(Y,0):MAX=10:GOSUB4480
...
4290 IFC1=0THEN4340
4300 E=0:ME$(Y,3)="":ME$(Y,4)="":FORX=1TOC1:ME$(Y,3)=ME$(Y,3)+MID$(STR$(OP(X)),2)+"{f4}":T=DX(OP(X)):IFT=0ORE>0THEN4320
4310 AA=LEN(ME$(Y,4)):FORI=1TOT:AA=AA+LEN(DF$(X,I)+"{f4}"):NEXTI:IFAA>254THENPRINTCHR$(13)"{down}The datafields are too large to compile!":E=1:GOTO4320
4315 FORI=1TOT:ME$(Y,4)=ME$(Y,4)+DF$(X,I)+"{f4}":NEXTI
4320 NEXTX:IFE>0THEN4340:ELSEME$(Y,3)=LEFT$(ME$(Y,3),LEN(ME$(Y,3))-1):IFLEN(ME$(Y,4))<>0THENME$(Y,4)=LEFT$(ME$(Y,4),LEN(ME$(Y,4))-1)
...
4510 PRINT"{down}Loading...";:CLOSE2:CLOSE15:OPEN15,X,15:OPEN2,X,2,MID$(STR$(Y),2)+":sys.variables,s,r":INPUT#15,A:IFA>0THENPRINT"Not found.":CLOSE2:CLOSE15:RETURN
...
5090 M6=VAL(M$(16)):IFM6<1ORM6>8THENPRINT"{clear}";:GOTO5530:ELSEPRINT"{clear}":FORX=1TOM6:A$=MID$(CC$,X,1):Y=INSTR("{white}{red}{cyan}{purple}{green}{blue}{yellow}{orange}{brown}{pink}{dark gray}{gray}{light green}{light blue}{light gray}",A$)
...
5430 PRINT"All system upcase flag":PRINT"(0=off, 1=on): "M$(4)"{left}";:INPUT#1,M$(4):PRINT:PRINT
5435 PRINT"Make user data in U)ppercase":PRINT"or F)irst mode (U/F): "M$(0)"{left}";:INPUT#1,M$(0):PRINT:IFINSTR("UFuf",M$(0))=0ORLEN(M$(0))>1THEN5435
5436 M$(0)=CHR$(ASC(M$(0))OR128)
...
5495 IFVAL(M$(16))<1ORVAL(M$(16))>8THENPRINT"Range is 1-8!":PRINT:GOTO5490
5497 PRINT"{down}Enter access levels":PRINT"for networked news.":PRINT"=>";:I$=MT$(8):GOSUB2440:MT$(8)=I$
5499 PRINT"{down}Enter access levels":PRINT"for anonymous option.":PRINT"=>";:I$=MT$(9):GOSUB2440:MT$(9)=I$
...
5502 PRINT"Sysop menu: ";:I$=MT$(5):GOSUB2010:MT$(5)=I$:PRINT"Menu of CMDS everywhere: ";:I$=MT$(7):GOSUB2010:MT$(7)=I$
...
5530 POKE2603,8:PRINTCHR$(27)"e{clear}";:GOTO5070
5540 PRINT"{clear}{down}   Max calls, time, idle, mins/day":OPEN1,0:PRINT:GOSUB5520
5550 FORX=1TO26:PRINTCHR$(X+192)"        "CT(X,0)","CT(X,1)","CT(X,2)","CT(X,3)
5555 PRINT"{up}{right*10}";:INPUT#1,CT(X,0),CT(X,1),CT(X,2),CT(X,3):PRINT:NEXTX:CLOSE1:PRINT"{clear}";:GOTO5530
5560 PRINT:GOSUB5520:GOSUB4500:PRINT"{clear}";:GOTO5530
5570 PRINT:GOSUB5520:GOSUB5580:PRINT"{clear}";:GOTO5530
5580 IFPEEK(215)=0THENPRINT"{cyan}{clear}";
...
5590 CLOSE15:OPEN15,X,15:PRINT#15,"s"+MID$(STR$(Y),2)+":sys.variables"
5600 PRINT"{down}Saving...";:CLOSE2:OPEN2,X,2,MID$(STR$(Y),2)+":sys.variables,s,w":INPUT#15,A:IFA>0THENPRINT"Error.":SLEEP2:CLOSE2:CLOSE15:RETURN
...
5695 PRINT"{reverse on} [U]pdate Upp/First user data  "
5700 PRINT"{reverse on}{cm p*31}"
5710 GETKEYA$:A$=CHR$(ASC(A$)AND127):I=INSTR("fachu",A$):IFITHENX=8:GOSUB4490:ONIGOTO5720,5930,5940,6100,6200:ELSEWINDOW0,1,PEEK(238),24,1:GOTO340
5715 POKE2603,8:PRINTCHR$(27)"e{clear}";:IFPEEK(238)=39THEN5640:ELSE5710
5720 GOSUB5520:I=255:FORX=1TO10:I=I-LEN(F$(X)):NEXTX:PRINT"{cyan*2}{clear}{cyan}{down}Function keys:{white}"STR$(I)" {cyan}bytes free.{down}"
...
5740 GOSUB930:GOSUB2010:IFLEN(I$)<=ITHENF$(T)=I$:GOTO5720:ELSEPRINT"{pink}Not enough space in fkey buffer":SLEEP2:PRINT"{cyan}{home}{down*15}"CHR$(27)"@{cyan}";:GOTO5740
...
5810 PRINT"{down}Saving...";:OPEN2,X,2,MID$(STR$(Y),2)+":sys.misc data":RECORD#2,2:A$="":FORX=1TO10:A$=A$+F$(X)+CHR$(13):NEXTX
5820 PRINT#2,A$;:PRINT"done.":CLOSE2:GOTO5720
5900 PRINT"{down}Insert disk containing "CHR$(34)"sys.misc data"CHR$(34):PRINT"Drive, Device/lu = 8,0{left*3}";:OPEN1,0:INPUT#1,X,Y:CLOSE1:PRINT
5910 PRINT"{down}Loading...";:OPEN2,X,2,MID$(STR$(Y),2)+":sys.misc data":RECORD#2,2:FORX=1TO10:SYS8222:F$(X)=I$
...
6000 IFPEEK(215)=0THENPRINT"{cyan}{clear}";
6003 E=0:PRINT"{pink}Note: {white}This function will destroy any":PRINT"      data currently in memory.":PRINT"{down}{cyan}Continue (y/n)? {reverse on} {reverse off}{left}";:GETKEYA$
6005 IFA$="y"THENPRINT"Yes":ELSEPRINT"No.":E=1:RETURN
6010 IFPEEK(215)=0THENPRINT"{cyan}{clear}";
6015 E=0:PRINT"{down}{cyan}Insert disk containing "CHR$(34)I$CHR$(34):PRINT"Drive, Device/lu = 8,0{left*3}";:OPEN1,0:INPUT#1,X,Y:CLOSE1:PRINT:PRINT"{home*2}{clear}";:RETURN
6050 IFPEEK(215)=0THENPRINT"{cyan}{clear}";
6055 E=0:PRINT"{down}{cyan}Insert disk containing "CHR$(34)I$CHR$(34):PRINT"Drive, Device/lu = 8,0{left*3}";:OPEN1,0:INPUT#1,X,Y:CLOSE1:PRINT:RETURN
6100 I$="sys.userfiles":GOSUB6050:OPEN2,X,2,MID$(STR$(Y),2)+":sys.userfiles":OPEN4,4,7
6105 PRINT#4,"ID#  HANDLE            REAL NAME         PHONE NUMBER   ACS E8WS?X#BC"
6107 PRINT#4,"---  ----------------  ----------------  -------------  --- ---------"
6110 I=1:DO:RECORD#2,I:SYS8222:IFI$="^"ORI$="{pound}"THEN6130
6115 LN$(995)=I$:SYS8222:FORX=996TO999:SYS8222:LN$(X)=I$:NEXTX:SYS8222:SYS8222:SYS8222:LN$(1000)=I$
6120 PRINT#4,USING"###  ################  ################  ##############  #  #########";MID$(STR$(I),2),LN$(995),LN$(996),LN$(997),LN$(998),LN$(999),LN$(1000)
6130 IFI$="{pound}"ORI>VAL(M$(15))THENEXIT
6140 I=I+1:LOOP:CLOSE2:CLOSE4:PRINT"{clear}";:GOTO5710
6200 PRINTCHR$(27)"s";:I$="sys.userfiles":GOSUB6050:W0$=M$(0):PRINT"{down}{cyan}System is set for ";M$(0);:IFM$(0)="U"THENPRINT"ppercase mode.{down}":ELSEPRINT"irst mode.{down}"
6210 PRINT"{down}{cyan}Begin this conversion (Y/N)? {reverse on} {reverse off}{left}";:GETKEYA$:IFA$="y"THENPRINT"Yes":ELSEPRINT"No.":GOTO5710
6220 PRINT"{down}{cyan}Working...";:GOTO6230
6225 FORY=1TOLEN(I$):A=ASC(MID$(I$,Y,1))
6226 IFW0$="F"ANDY>1THENIFMID$(I$,Y-1,1)<>" "ANDA>128THENMID$(I$,Y,1)=CHR$(AAND127)
6227 IFW0$="U"ANDA>64ANDA<91THENMID$(I$,Y,1)=CHR$(AOR128)
6228 NEXTY:A$=A$+I$+CHR$(13):RETURN
6230 OPEN2,X,2,MID$(STR$(Y),2)+":sys.userfiles":I=1:DO:RECORD#2,I:RECORD#2,I:SYS8222:IFI$="{pound}"THENEXIT:ELSEIFI$="^"THEN6310
6240 A$="":GOSUB6225:SYS8222:GOSUB6225:SYS8222:GOSUB6225
6250 FORX=4TO8:SYS8222:A$=A$+I$+CHR$(13):NEXTX
6260 FORX=1TO8:GET#2,I$:A$=A$+CHR$(ASC(I$+CHR$(0))):NEXTX
6270 FORX=1TO16:GET#2,I$:A$=A$+CHR$(ASC(I$+CHR$(0))):NEXTX
6300 RECORD#2,I:RECORD#2,I:PRINT#2,A$
6310 PRINT".";:I=I+1:LOOP:CLOSE2:CLOSE1:GOTO5710
