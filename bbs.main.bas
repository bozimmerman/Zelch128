...
8015 IFLEN(SY$)>1ANDLEN(SY$)<4ANDLEFT$(SY$,1)="#"THENI=VAL(MID$(SY$,2)):U=D(I,0):D=D(I,1):GOSUB990:GOTO8025:ELSEU=VAL(SY$):GOSUB990:D=VAL(SY$):GOTO8025
8020 GOSUB990:O$=T$(4):GOSUB4:POKE2895,1:POKE2847,2:GOSUB10:D=I:IFD<0ORD>9ORI$=""THEND=0
8025 IFLEN(SY$)>4ANDINSTR(SY$,"/")>0THEND$=SY$:RETURN:ELSED$=MID$(STR$(D),2):RETURN
8030 GOSUB8000:IFETHENGOSUB990:GOSUB990:RETURN:ELSEGOSUB8010:RETURN
8040 GOSUB8000:ONEGOTO920:O$(15)=O$(16):GOTO900
8050 GOSUB8030:ONEGOTO920:OPEN2,U,2,D$+":"+F$+",p,r":GET#2,A$:IFSTTHENCLOSE2:O$=T$(5):GOSUB4:GOTO920:ELSECLOSE2:I$=F$:GOTO23
...
8330 O(11)=0:O$(5)="":L=1:OPEN1,U,15:OPEN2,U,2,D$+":"+F$+",s,r":INPUT#1,I:IFI>20THENCLOSE1:CLOSE2:O$=T$(16):GOSUB4:O(12)=1:RETURN
...
8415 IFX=0ANDSY$<>"+"THENGOSUB25:GOTO940:ELSEOPEN1,U,15:OPEN2,U,2,D$+":"+F$+",s,r":IFX>0THENFORI=1TOX:SYS8213:NEXTI:IFPEEK(253)THEN8435
...
8620 GOSUB8010:GOSUB990:I=0:IFSY$="*"THENGOSUB8650:GOTO8630
8625 IFSY$="#"THENW1$=CHR$(UB(1)+192)+W1$:I=UB(1)+192:U=D(8,0):D=D(8,1):D$=MID$(STR$(D),2):ELSEI=VAL(SY$)
8630 OPEN2,U,0,"$"+D$+":"+W1$:POKE253,I:SYS8219:CLOSE2:GOTO940
...
8800 GOSUB8010:OPEN2,U,0,"$"+D$+":"+W1$:SYS8228:CLOSE2:O$="{ct k}{black}"+MID$(STR$(PEEK(2858)+256*PEEK(2859)),2)+" files counted.":GOSUB4:GOTO940
...
8970 POKE2931,0:O$(11)="":U=D(0,0):D=D(0,1):IFPEEK(2894)=0THENTRAP8975:DCLEARU(D(0,0)):ELSEIFPEEK(2961)<2THEN940:ELSEI$="prg.monster":GOTO23
