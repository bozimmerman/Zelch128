!--------------------------------------------------
!- Tuesday, May 16, 2017 10:30:04 PM
!- Import of : 
!- c:\src\zelch128next\prg.network.prg
!- Commodore 128 BASIC 7/7.1
!--------------------------------------------------
30000 SYS15625:REM ============== NETWORK MODULE V2.2 ============== 12/07/91 =
30010 IFW1=.1THEN50000:ELSEIFW1=.2THEN51000
30015 ON(W1-700)GOTO31000,32000,33000,34000,35000,36000:GOTO920
30020 I$="":TI$="000000"
...
33000 W1=1:GOSUB760:W3$=LEFT$(SY$,1):IFI>0ANDI<27THENW1=I
...
34001 NEXTI:O$="{ct k}{black}Enter {f5}"+A$+"{f5} as the system{f7}name to re-enter your host data.":GOSUB4:FORI=1TO7:TT$(I)=" 0000000000":NEXTI
34006 O$="{ct k}{black}System name: {ct c}":GOSUB4:X=20:GOSUB16:TT$(1)=I$:Y=0:IFTT$(1)=A$THENY=1:O$="{ct k}Re-Entering host data.":GOSUB4:ELSEO$="Entering new system":GOSUB4
34010 O$="{ct k}{black}Network password: {ct c}":GOSUB4:X=10:GOSUB16:TT$(2)=I$:IFI$=""THEN34010
34020 O$="{ct k}{black}Actual phone number:":GOSUB4:GOSUB34500:TT$(3)=W1$
34030 IFYTHEN34070:ELSEO$="{ct k}{black}Network to number:":GOSUB4:GOSUB34500:TT$(4)=W1$
34040 O$="{ct k}{black}Baud rate: {ct c}":GOSUB4:X=4:GOSUB5:IFI$<>"300"ANDI$<>"1200"ANDI$<>"2400"THEN34040:ELSETT$(5)=MID$(STR$(VAL(I$)/300),2)
34050 O$="{ct k}{black}Accept network news from this BBS (y/n)? {ct c}":GOSUB4:C$="NY":GOSUB28:TT$(6)=MID$(STR$(I-1),2)
34055 O$="{ct k}{black}Send network news to this BBS (y/n)? {ct c}":GOSUB4:C$="NY":GOSUB28:TT$(6)=TT$(6)+MID$(STR$(I-1),2)
34060 O$="{ct k}{black}Network which subboard(s){f7}A-Z or 1-26 separated by semicolons{ct k}: {ct c}":GOSUB4:X=40:GOSUB16:TT$(7)="00000000000000000000000000":IFI$=""THEN34070
34063 DO:IFVAL(I$)=0THENA=ASC(LEFT$(I$,1))-192:I$=MID$(I$,2):GOTO34067:ELSEA=VAL(I$):X=INSTR(I$,";"):IFX=0THENX=LEN(I$)
34065 I$=MID$(I$,X+1)
34067 IFA>0ANDA<27THENMID$(TT$(7),A,1)="1"
34069 LOOPUNTILI$=""
34070 O$="{ct k}{black}S)ave, R)e-enter, A)bort: {white}{ct c}":GOSUB4:C$="SRA":GOSUB28:ONIGOTO34080,34000,920
34080 OPEN2,D(1,0),2,MID$(STR$(D(1,1)),2)+":sys.network":X=27:IFY=0THENX=0:DO:X=X+1:RECORD#2,X:SYS8222:IFLEN(I$)>2THEN34100
34090 A$="":FORI=1TO7:A$=A$+TT$(I)+R$:NEXTI:RECORD#2,X:PRINT#2,A$;:IFY=0THENEXIT:ELSECLOSE2:GOTO940
...
36050 GOSUB400:I=W8:GOSUB52250:A$=I$:I=W9:GOSUB52250:O$(5)=A$+R$+I$+R$+"NEWS"+R$+O$(24):U=D(1,0):D=D(1,1):F$="sys.news":GOSUB1210:GOTO940
50000 REM ----------- NETWORK RECEIVE ROUTINES --------------------------------
50001 DO:GET#5,A$:LOOPUNTILA$="":O(19)=0:O$(23)="Punter":BLOAD"p-xfer 1300",U(D(0,0)),D(D(0,1)):GOSUB51310:Y=1:PRINT#5,"GGGGGG";
...
51010 MB(5)=0:O(19)=0:BLOAD"p-xfer 1300",U(D(0,0)),D(D(0,1)):O$(23)="Punter":A$="e0":GOSUB2:GOSUB51310:O$="Rummaging through files...":GOSUB250
